# CLAUDE.md — Privacy Panel

## Project Overview

Privacy Panel is an open-source tool that parses company privacy policies, extracts structured data into a standardized JSON schema, generates a consumer-facing "Privacy Panel" label (modeled on FDA Nutrition Facts), and applies a transparent, versioned rubric to produce a letter grade (A–F).

The project has three layers:
1. **Factual extraction** — AI-powered parsing of privacy policies into structured data
2. **Neutral label** — A standardized visual disclosure (the "Privacy Panel" panel)
...
│   │   ├── schema/
│   │   │   ├── privacy-panel.schema.ts    # Zod schema for the Privacy Panel JSON
...
│
├── cli/
│   └── index.ts                       # CLI entry: `privacypanel analyze <url>`
│
├── tests/
│   ├── core/
│   │   ├── extraction.test.ts
...
Create the Privacy Panel JSON schema using Zod. This is the contract that everything else depends on.

**Files:** `src/core/schema/privacy-panel.schema.ts`, `src/core/schema/types.ts`

**Requirements:**
- Define the full PrivacyPanel Zod schema matching the spec below
...
- `score(extraction: PrivacyPanel, rubric: Rubric): GradeResult`
...
**Requirements:**
- `extract(policyText: string, companyName?: string): Promise<PrivacyPanel>`
...
- Check for `.well-known/privacy-panel.json` at the domain root first (if it exists and validates, use it directly — skip AI extraction)
...
Build the React components for the Privacy Panel label.

**Files:** `src/core/rendering/PrivacyPanelLabel.tsx`, `src/core/rendering/GradedLabel.tsx`, `src/core/rendering/ComparisonView.tsx`

**Requirements:**
- Port the label design from the existing React artifact (the FDA-style Nutrition Facts label)
- `PrivacyPanelLabel` accepts a `PrivacyPanel` object and renders the neutral label
- `GradedLabel` accepts `PrivacyPanel` + `GradeResult` and renders with the grade header
...
- Title: "Privacy Panel" in 32pt+ bold Helvetica/Arial
...
**Requirements:**
- `privacypanel analyze <url>` — fetch, extract, score, print results
- `privacypanel analyze <url> --json` — output raw JSON
- `privacypanel analyze <url> --label` — output SVG label to stdout
- `privacypanel score <json-file>` — score an existing extraction
- `privacypanel validate <json-file>` — validate a Privacy Panel JSON file
...
- "The grade reflects Privacy Panel' assessment based on our published rubric (v{version}). It is our opinion."
